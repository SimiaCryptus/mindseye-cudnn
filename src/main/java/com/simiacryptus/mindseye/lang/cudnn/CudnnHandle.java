...
public class CudnnHandle extends CudaDevice {
  ...
  public CudaTensorList add(final CudaTensorList left, final CudaTensorList right) {
    ...
    @Nonnull
    final CudaResource<cudnnOpTensorDescriptor> opDescriptor = newOpDescriptor(cudnnOpTensorOp.CUDNN_OP_TENSOR_ADD,
        precision);
    @Nonnull
    ...
    if (null != left)
      left.freeRef();
    @Nonnull
    final CudaMemory outputPtr = allocate((long) outputDescriptor.nStride * precision.size * length,
    ...
    CudaTensorList temp_53_0001 = new CudaTensorList(new CudaTensor(outputPtr == null ? null : outputPtr,
        outputDescriptor == null ? null : outputDescriptor, precision), length, dimensions, precision);
    return temp_53_0001;
  }
  ...
  public CudaTensorList addInPlace(final CudaTensorList left, final TensorList right) {
    assert CudaDevice.isThreadDeviceId(getDeviceId());
    @Nullable
    final CudaTensor lPtr = getTensor(left == null ? null : left.addRef(), left.getPrecision(), MemoryType.Device,
        false);//.moveTo(gpu.getDeviceNumber());
    @Nullable
    final CudaTensor rPtr = getTensor(right == null ? null : right.addRef(), left.getPrecision(), MemoryType.Device,
        false);//.moveTo(gpu.getDeviceNumber());
    if (null != right)
      right.freeRef();
    assert lPtr != null;
    ...
    try {
      ...
      if (data instanceof CudaTensorList) {
        ...
        if (precision == cudaTensorList.getPrecision()) {
          CudaTensor temp_53_0003 = this.getTensor(cudaTensorList == null ? null : cudaTensorList.addRef(), memoryType,
              dense);
          ...
          String msg = RefString.format(
              ...
              Integer.toHexString(com.simiacryptus.ref.wrappers.RefSystem.identityHashCode(cudaTensorList)),
      ...
      @Nonnull
      final CudaMemory ptr = this.allocate((long) elementLength * listLength * precision.size, memoryType, true);
      ...
      @Nonnull
      final CudaDevice.CudaTensorDescriptor descriptor = newTensorDescriptor(precision, data.length(), channels, height,
          width, channels * height * width, height * width, width, 1);
      CudaTensor temp_53_0004 = new CudaTensor(ptr == null ? null : ptr, descriptor == null ? null : descriptor,
          precision);
      return temp_53_0004;
    } finally {
      data.freeRef();
  ...
  public TensorList addAndFree(final Precision precision, final TensorList left, final TensorList right) {
    ...
    final CudaDevice.CudaTensorDescriptor outputDescriptor = newTensorDescriptor(precision, length, dimensions[2],
        ...
        dimensions[0], 1);
    @Nullable
    final CudaTensor lPtr = getTensor(left == null ? null : left.addRef(), precision, MemoryType.Device, false);//.moveTo(gpu.getDeviceNumber());
    if (null != left)
      left.freeRef();
    @Nullable
    final CudaTensor rPtr = getTensor(right == null ? null : right.addRef(), precision, MemoryType.Device, false);//.moveTo(gpu.getDeviceNumber());
    if (null != right)
      right.freeRef();
    assert lPtr.descriptor.batchCount == rPtr.descriptor.batchCount;
    assert lPtr.descriptor.channels == rPtr.descriptor.channels;
    ...
    assert lPtr.descriptor.width == rPtr.descriptor.width;
    @Nonnull
    final CudaMemory outputPtr = allocate(outputDescriptor.nStride * length * precision.size, MemoryType.Device, true);
    CudaMemory lPtrMemory = lPtr.getMemory(this);
    CudaMemory rPtrMemory = rPtr.getMemory(this);
    assert CudaDevice.isThreadDeviceId(getDeviceId());
    cudnnOpTensor(opDescriptor.getPtr(), precision.getPointer(1.0), lPtr.descriptor.getPtr(), lPtrMemory.getPtr(),
    ...
    if (null != rPtrMemory)
      rPtrMemory.freeRef();
    RefUtil.freeRef(outputPtr.dirty());
    CudaTensorList temp_53_0005 = new CudaTensorList(new CudaTensor(outputPtr == null ? null : outputPtr,
        outputDescriptor == null ? null : outputDescriptor, precision), length, dimensions, precision);
    return temp_53_0005;
  }
