...
public @RefAware
class ImgLinearSubnetLayer extends LayerBase implements MultiPrecision<ImgLinearSubnetLayer> {
  ...
  public Result eval(@Nonnull final Result... inObj) {
    ...
    Result[] legResults;
    legResults = legs.stream().map(RefUtil.wrapInterface(
        (Function<? super ImgLinearSubnetLayer.SubnetLeg, ? extends Result>) leg -> {
          ImgBandSelectLayer imgBandSelectLayer = new ImgBandSelectLayer(leg.fromBand, leg.toBand);
          Result temp_06_0011 = imgBandSelectLayer
              .eval(input == null ? null : input.addRef());
          ...
          Result temp_06_0006 = leg.inner
              .eval(new Result(legData, new Result.Accumulator() {
                {
                }
    ...
    Result temp_06_0005 = sumInputsLayer
        .eval(Result.addRefs(legResults));
